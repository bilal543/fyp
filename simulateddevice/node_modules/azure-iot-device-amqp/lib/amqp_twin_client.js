...
var AmqpTwinClient = /** @class */ (function (_super) {
            ...
            states: {
                'detached': {
                    ...
                    handleErrorEmit: function (err) {
                        ...
                        _this.emit(AmqpTwinClient.errorEvent, azure_iot_amqp_base_1.translateError('received an error from the amqp transport: ', err));
                    },
                    handleLinkDetach: function (detachObject) {
                        if (detachObject && detachObject.error) {
                            _this.emit(AmqpTwinClient.errorEvent, azure_iot_amqp_base_1.translateError('received an error from the amqp transport: ', detachObject.error));
                        }
                    },
                            ...
                            if (err) {
                                ...
                                _this._client.attachReceiverLink(_this._endpoint, _this._generateTwinLinkProperties(linkCorrelationId_1), function (receiverLinkError, receiverTransportObject) {
                                    if (receiverLinkError) {
                                        ...
                                        _this._downstreamAmqpLink.on('error', _this._handleError.bind(_this));
                                        _this._client.attachSenderLink(_this._endpoint, _this._generateTwinLinkProperties(linkCorrelationId_1), function (senderLinkError, senderTransportObject) {
                                            if (senderLinkError) {
                                                ...
                                                _this._upstreamAmqpLink.on('error', _this._handleError.bind(_this));
                ...
                'attached': {
                    ...
                    actual_handleNewListener: function (eventName) {
                        if (eventName === AmqpTwinClient.responseEvent) {
                            ...
                            _this.emit(AmqpTwinClient.subscribedEvent, { 'eventName': AmqpTwinClient.responseEvent, 'transportObject': _this._upstreamAmqpLink });
                        }
                            ...
                            _this._internalOperations[correlationId] = function () {
                                _this.emit(AmqpTwinClient.subscribedEvent, { 'eventName': AmqpTwinClient.postEvent, 'transportObject': _this._upstreamAmqpLink });
                            };
                    ...
                    actual_handleRemoveListener: function (eventName) {
                        if ((eventName === AmqpTwinClient.postEvent) && events_1.EventEmitter.listenerCount(_this, AmqpTwinClient.postEvent) === 0) {
                        ...
                        if ((events_1.EventEmitter.listenerCount(_this, AmqpTwinClient.postEvent) + events_1.EventEmitter.listenerCount(_this, AmqpTwinClient.responseEvent)) === 0) {
                    ...
                    detach: function (callback) {
                        _this._fsm.transition('detaching', callback);
                    },
                    _onExit: function () {
                ...
                'detaching': {
                    _onEnter: function (detachCallback) {
                        _this._client.detachSenderLink(_this._endpoint, function (detachSenderError, result) {
                            ...
                            _this._client.detachReceiverLink(_this._endpoint, function (detachReceiverError, result) {
                                ...
                                _this._fsm.transition('detached', null, detachCallback);
                            });
                        });
                    },
                    handleErrorEmit: function () {
     ...
     */
    AmqpTwinClient.prototype.sendTwinRequest = function (method, resource, properties, body, done) {
        this._fsm.handle('sendTwinRequest', method, resource, properties, body, done);
    };
    AmqpTwinClient.prototype.attach = function (callback) {
    ...
    AmqpTwinClient.prototype._handleNewListener = function (eventName) {
        this._fsm.handle('handleNewListener', eventName);
    };
    AmqpTwinClient.prototype._handleRemoveListener = function (eventName) {
        this._fsm.handle('handleRemoveListener', eventName);
    };
    AmqpTwinClient.prototype._onAmqpMessage = function (message) {
        //
    ...
    AmqpTwinClient.prototype._onAmqpDetached = function (detachObject) {
        this._fsm.handle('handleLinkDetach', detachObject);
    };
    AmqpTwinClient.prototype._handleError = function (err) {
        this._fsm.handle('handleErrorEmit', err);
    };
    AmqpTwinClient.prototype._safeCallback = function (callback, error, result) {
        if (callback) {
            process.nextTick(function () { return callback(error, result); });
        }
    };
    AmqpTwinClient.prototype._appendEventQueue = function () {
        var argArray = [];
    ...
    AmqpTwinClient.prototype._handleHeadOfEventQueue = function () {
        var head = this._eventQueue.shift();
        if (head) {
            this._fsm.handle.apply(this._fsm, head);
        }
    };
    AmqpTwinClient.prototype._AppendOrHandleEvent = function () {
        this._appendEventQueue();
        if (this._eventQueue.length === 1) {
            this._handleHeadOfEventQueue();
        }
    };
    AmqpTwinClient.prototype._isString = function (obj) {
        return (typeof obj === 'string');
    };
    AmqpTwinClient.prototype._isNumber = function (obj) {
        return (typeof obj === 'number');
    };
    AmqpTwinClient.prototype._isBoolean = function (obj) {
        return (typeof obj === 'boolean');
    };
    AmqpTwinClient.prototype._rundown_sendTwinRequest = function (method, resource, properties, body, done) {
        if (!!done && (typeof done === 'function')) {
            done(this._eventQueueError || new Error('Link Detached'));
        }
    };
    AmqpTwinClient.prototype._sendTwinRequest = function (method, resource, properties, body, done) {
        var _this = this;
        ...
        this._upstreamAmqpLink.send(amqpMessage, function (err, state) {
            if (err) {
    ...
    AmqpTwinClient.errorEvent = 'error';
    AmqpTwinClient.responseEvent = 'response';
    AmqpTwinClient.postEvent = 'post';
    AmqpTwinClient.subscribedEvent = 'subscribed';
